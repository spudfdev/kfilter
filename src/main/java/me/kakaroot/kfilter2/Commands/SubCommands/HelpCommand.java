...
public class HelpCommand implements SubCommand {
    ...
    private void sendHelpMessage(CommandSender sender) {
        if (sender instanceof Player) {
            ...
            sender.sendMessage(Utility.tcc("&a/kf add <word> <replacement> &7- Adds a word to the filter with its replacement &4[kf.add]"));
            sender.sendMessage(Utility.tcc("&a/kf remove <word> &7- Removes a word from the filter &4[kf.remove]"));
            sender.sendMessage(Utility.tcc("&a/kf list &7- Lists all filtered words &4[kf.list]"));
            sender.sendMessage(Utility.tcc("&a/kf reload &7- Reloads the config file &4[kf.reload]"));
            sender.sendMessage(Utility.tcc("&a/kf clearchat &7- Clears the chat &4[kf.clearchat]"));
